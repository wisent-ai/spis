...
fn attempt_root(
    ...
    let attempt = value
        ...
        .context("desktop runtime manifest has no typed u32 attempt coordinate")?;
    let attempt_id = value
        .get("attempt_id")
        .and_then(Value::as_str)
        .context("desktop runtime manifest has no typed attempt_id coordinate")?;
    for (name, component) in [
        ...
        safe_job_value(component, &format!("runtime manifest {name}"))?;
    }
    ...
    for (name, observed, leaf) in [
        ...
        if observed != expected {
            bail!("desktop runtime manifest {name} must equal exact canonical coordinate {expected:?}");
        }
    }
    Ok(base
