...
fn attempt_root(
    ...
    let attempt = value
        ...
        .context("mobile runtime manifest has no typed u32 attempt coordinate")?;
    let attempt_id = value
        .get("attempt_id")
        .and_then(Value::as_str)
        .context("mobile runtime manifest has no typed attempt_id coordinate")?;
    ...
    for (name, observed, leaf) in [
        ...
        if observed != expected {
            bail!("mobile runtime manifest {name} must equal exact canonical coordinate {expected:?}");
        }
    }
    Ok(base
